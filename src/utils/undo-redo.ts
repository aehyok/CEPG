...
export default <IState = any>(
  options: IDvaUndoRedoPluginOptions = defaultOptions,
) => {
  ...
  return {
        ...
        const result = immer(
          ...
          (patches: Patch[], inversePatches: Patch[]) => {
            ...
            if (patches.length) {
              ...
              if (
                newOptions.include.includes(namespace) &&
                !namespace.includes('@@')
              ) {
                ...
                if (action.clear === true) {
                  ...
                  if (stackItem) {
                    const {
                      patches: itemPatches,
                      inversePatches: itemInversePatches,
                    } = stackItem;
     ...
     */
    onReducer(reducer: Function): Function {
